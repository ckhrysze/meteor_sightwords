...
if (Meteor.isClient) {
  ...
  Template.checklists.checklistgroups = function() {
    var sets = [
      ...
      "but not what all were we when your can said thees use an each which",
      ...
      "old any same tell bot follow came want show also around form three small set",
      "put end does another well large muust big even such because turn here why ask",
  ...
  var selectWord = function(event) {
    ...
    selected[this] = event.srcElement.checked == true;
  ...
  Template.checklistgroup.events = {
    ...
    "change .wordListTitle": function(event) {
      $(event.srcElement).parents('.wordlist-container').find('input.wordcheck').click();
    }
  };

  var swapBody = function(newNode) {
